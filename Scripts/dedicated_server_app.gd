...
func _ready() -> void:
	...
	if game_server == null:
		status_label.text = "Status: SERVER MODULE MISSING"
		error_label.text = "Could not find /root/GameServer."
		_refresh_buttons()
		return
...
func _build_header() -> PanelContainer:
	var panel := _make_panel(COLOR_SURFACE, COLOR_BORDER, Vector2(0, 170), 24)
	...
	subtitle.text = "Dedicated Server"
...
func _build_controls_panel() -> PanelContainer:
	var panel := _make_panel(Color(0.058, 0.066, 0.079), COLOR_BORDER, Vector2(0, 102), 22)
	...
	start_button = _make_button("Start Server", COLOR_ACCENT, Vector2(150, 38))
	start_button.pressed.connect(_on_start_pressed)
	controls.add_child(start_button)

	stop_button = _make_button("Stop Server", COLOR_DANGER, Vector2(150, 38))
...
func _connect_game_server_signals() -> void:
	game_server.status_changed.connect(_on_status_changed)
	game_server.players_changed.connect(_on_players_changed)
	game_server.server_error.connect(_on_server_error)


func _refresh_all() -> void:
	_on_status_changed(game_server.get_status_text())
	_on_players_changed(game_server.players)


func _on_start_pressed() -> void:
	error_label.text = ""
	if game_server != null:
		game_server.start_server(game_server.DEFAULT_PORT)
...
func _on_status_changed(status_text: String) -> void:
	...
	_refresh_ip_label()
	summary_label.text = game_server.get_connection_summary()
	_refresh_buttons()


func _on_players_changed(_players: Dictionary) -> void:
	players_text.text = game_server.get_players_text()
	summary_label.text = game_server.get_connection_summary()
...
func _on_server_error(message: String) -> void:
	...
	summary_label.text = game_server.get_connection_summary()
	_update_status_chip(game_server.get_status_text())
	_refresh_buttons()


func _refresh_buttons() -> void:
	var server_online: bool = game_server != null and bool(game_server.is_running)
	start_button.visible = not server_online
	start_button.disabled = server_online
	stop_button.visible = server_online


func _refresh_ip_label() -> void:
	if game_server == null:
		ip_label.text = "Server module unavailable."
		return

	ip_label.text = "Port %d | Current IP for players: %s" % [game_server.DEFAULT_PORT, game_server.get_lan_ip_share_text()]
